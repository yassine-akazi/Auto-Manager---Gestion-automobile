    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-6 sm:mb-8">
            ...
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                    <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-primary mb-1">Modifier la Voiture</h1>
                    <p class="text-xs sm:text-sm text-secondary">{{ $car->marque }} {{ $car->modele }} ({{ $car->annee }})</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-card rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden border-2 border-card">
            ...
            <form action="{{ route('cars.update', $car) }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4 sm:space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div class="relative">
                            ...
                            <div class="custom-select-container">
                                <div id="marque-display" class="w-full bg-card border-2 border-card text-primary rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 cursor-pointer hover:border-red-800 transition-all flex items-center justify-between text-sm sm:text-base" onclick="toggleMarqueDropdown()">
                                    <span id="marque-selected-text" class="text-primary truncate">{{ $car->marque }}</span>
                                ...
                                <input type="hidden" id="marque" name="marque" value="{{ $car->marque }}" required>
                            </div>
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <div class="custom-select-container">
                                <div id="modele-display" class="w-full bg-card border-2 border-card text-primary rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 cursor-pointer hover:border-red-800 transition-all flex items-center justify-between text-sm sm:text-base" onclick="toggleModeleDropdown()">
                                    <span id="modele-selected-text" class="text-primary truncate">{{ $car->modele }}</span>
                                ...
                                <input type="hidden" id="modele" name="modele" value="{{ $car->modele }}" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div class="relative">
                            ...
                            <input type="number" name="annee" value="{{ old('annee', $car->annee) }}" placeholder="Ex: 2023" min="1900" max="2026" class="w-full bg-card border-2 border-card text-primary rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 hover:border-red-800 placeholder-gray-500 text-sm sm:text-base" required>
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="kilometrage" value="{{ old('kilometrage', $car->kilometrage) }}" placeholder="Ex: 45000" min="0" class="w-full bg-card border-2 border-card text-primary rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 hover:border-red-800 placeholder-gray-500 text-sm sm:text-base" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4 sm:space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div class="relative">
                            ...
                            <div class="relative">
                                <input type="number" name="prix_achat" step="0.01" value="{{ old('prix_achat', $car->prix_achat) }}" placeholder="Ex: 150000" class="w-full bg-card border-2 border-card text-primary rounded-lg sm:rounded-xl pl-10 sm:pl-12 pr-3 sm:pr-4 py-2 sm:py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 hover:border-red-800 placeholder-gray-500 text-sm sm:text-base" required>
                        ...
                        <div class="relative">
                            ...
                            <div class="relative">
                                <input type="number" name="prix_vente" step="0.01" value="{{ old('prix_vente', $car->prix_vente) }}" placeholder="Ex: 180000" class="w-full bg-card border-2 border-card text-primary rounded-lg sm:rounded-xl pl-10 sm:pl-12 pr-3 sm:pr-4 py-2 sm:py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 hover:border-red-800 placeholder-gray-500 text-sm sm:text-base">
                                <div class="absolute left-3 sm:left-4 top-2.5 sm:top-3.5 text-red-400 font-semibold text-xs sm:text-base">DH</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <input type="date" name="date_dachat" value="{{ old('date_dachat', $car->date_dachat) }}" class="w-full bg-card border-2 border-card text-primary rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 hover:border-red-800 text-sm sm:text-base" required>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <div class="md:hidden space-y-3">
                            <div>
                                ...
                                <input type="text" name="matricule_part1" placeholder="20003" value="{{ old('matricule_part1', $car->matricule_part1) }}" 
                            ...
                            <div>
                                ...
                                <input type="text" name="matricule_part2" placeholder="B" value="{{ old('matricule_part2', $car->matricule_part2) }}" maxlength="1" 
                            ...
                            <div>
                                ...
                                <input type="text" name="matricule_part3" placeholder="33" value="{{ old('matricule_part3', $car->matricule_part3) }}" 
                                       class="w-full bg-card border-2 border-card text-primary rounded-lg px-4 py-3 text-center text-lg font-bold focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 hover:border-red-800 placeholder-gray-500" required>
                            </div>
                        </div>
                        ...
                        <div class="hidden md:flex gap-3 items-center">
                            <input type="text" placeholder="20003" value="{{ old('matricule_part1', $car->matricule_part1) }}" 
                            ...
                            <input type="text" placeholder="B" value="{{ old('matricule_part2', $car->matricule_part2) }}" maxlength="1" 
                            ...
                            <span class="text-2xl text-red-500 font-bold">-</span>
                            <input type="text" placeholder="33" value="{{ old('matricule_part3', $car->matricule_part3) }}" 
                                   class="flex-1 bg-card border-2 border-card text-primary rounded-xl px-4 py-3 text-center text-lg font-bold focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 hover:border-red-800 placeholder-gray-500" 
                    ...
                    <div>
                        ...
                        <select name="statut" class="w-full bg-card border-2 border-card text-primary rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 hover:border-red-800 appearance-none text-sm sm:text-base custom-select">
                            @foreach(['Disponible','Réservée','En réparation','En inspection','En négociation','Vendue','Hors stock'] as $status)
                                <option value="{{ $status }}" {{ old('statut', $car->statut) == $status ? 'selected' : '' }}>{{ $status }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>

                {{-- Section 4: Photo Actuelle --}}
                @if($car->image)
                <div class="space-y-4 sm:space-y-6">
                    <div class="flex items-center gap-2 sm:gap-3 pb-3 sm:pb-4 border-b-2 border-card">
                    ...
                    <div class="max-w-md mx-auto">
                        <div class="aspect-video rounded-lg sm:rounded-xl overflow-hidden border-2 border-card shadow-lg">
                            <img src="{{ asset('uploads/cars/' . $car->image) }}" alt="Photo actuelle" class="w-full h-full object-cover">
                        </div>
                        <p class="mt-2 text-xs sm:text-sm text-secondary text-center">
                            <svg class="w-4 h-4 inline text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            Ajoutez une nouvelle photo ci-dessous pour remplacer celle-ci
                        </p>
                    </div>
                </div>
                @endif

                {{-- Section 5: Nouvelle Photo --}}
                <div class="space-y-4 sm:space-y-6">
                    <div class="flex items-center gap-2 sm:gap-3 pb-3 sm:pb-4 border-b-2 border-card">
                        <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-red-600 to-red-900 rounded-lg sm:rounded-xl flex items-center justify-center text-white font-bold shadow-lg shadow-red-900/50 text-sm sm:text-base">{{ $car->image ? '5' : '4' }}</div>
                        <h2 class="text-xl sm:text-2xl font-bold text-primary flex items-center gap-2">
                            ...
                            <span class="hidden sm:inline">{{ $car->image ? 'Changer la Photo' : 'Ajouter une Photo' }}</span>
...
const currentMarque = "{{ $car->marque }}";
const currentModele = "{{ $car->modele }}";
